...
public class NotesList extends ListActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        getListView().setBackgroundColor(Color.parseColor("#1f39a5fd"));
        ...
        View layoutView = getLayoutInflater().inflate(R.layout.item_search, null);
        final EditText editTitle = (EditText) layoutView.findViewById(R.id.title);
        Button buttonSearch = (Button) layoutView.findViewById(R.id.search);
        buttonSearch.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                //获取搜索框的内容
                final String searchContent = editTitle.getText().toString().trim();
                //根据标题搜索
                String search_selection = "title LIKE '%" + searchContent + "%'";
                Cursor cursor = managedQuery(
                        getIntent().getData(),            // Use the default content URI for the provider.
                        PROJECTION,                       // Return the note ID and title for each note.
                        search_selection,                             // No where clause, return all records.
                        null,                             // No where clause, therefore no where column values.
                        NotePad.Notes.DEFAULT_SORT_ORDER  // Use the default sort order.
                );
                SimpleCursorAdapter adapter
